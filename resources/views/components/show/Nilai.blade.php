@extends('layouts.app')
@section('title', 'Kelas ' . $class->name_class)
@section('content')
    ...
    <div class="p-4 sm:ml-64">
        <div class="p-4 border-dashed rounded-lg dark:border-gray-700 mt-10">
            ...
            @endif
            <a href="" onclick="event.preventDefault(); window.location.href='{{ url('nilai') }}'"
                    ...
                    <ul class="my-4 space-y-3">
                        <li>
                            <div
                                ...
                                <span class="flex-1 ml-3 whitespace-nowrap">{{ $item->mapel }}</span>
                                <div class="relative max-w-sm">
                                    ...
                                    <form action="{{ route('nilai.date', [$class->uu_class, $item->uu_mapel]) }}" method="GET">
                                        <div class="flex">
                                            <input datepicker datepicker-autohide type="text" name="date"
                                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                                placeholder="Pilih Tanggal">
                                            <button type="submit"
                                                class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 ml-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">Submit</button>
                                        </div>
                                    </form>
                                </div>
                            </div>
            ...
            <script>
                $(document).ready(function() {
                    // Mendeteksi perubahan pada input tanggal
                    $('#tanggal').on('change', function() {
                        var selectedDate = $(this).val();
                        if (selectedDate) {
                            // Mengarahkan pengguna ke halaman dengan tanggal yang dipilih
                            window.location.href = "/page?tanggal=" + selectedDate;
                        }
                    });
                });
            </script>
