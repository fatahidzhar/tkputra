...
@section('content')
    ...
    <div class="pr-4 pl-4">
        <form action="{{ route('kritiksaran.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="mb-5">
                <div>
                    <label for="religion" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Kritik &
                        Saran</label>
                    <input type="hidden" name="uuid_m" value="{{ Auth::user()->uuid }}">
                    <select id="religion" name="uu_categori"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option selected>Belum Pilih</option>
                        @foreach ($categories_parent as $item)
                            <option value="{{ $item->uu_categori }}">{{ $item->name }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-6 mt-4">
                    <label for="faq" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Tulis Kritik
                        & Saran</label>
                    <textarea id="faq" name="text" rows="4"
                        class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"></textarea>
                </div>
                <button type="submit"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Kirim
                    Kritik & Saran</button>
            </div>
        </form>
        @if ($errors->any())
                            ...
                            <blockquote class="text-md mt-3 italic font-semibold text-gray-900 dark:text-white">
                                <div class="flex justify-between align-middle">
                                    ...
                                    <div>
                                        @if ($item->status1 == 1)
                                            <span
                                                class="bg-green-100 text-green-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-green-300">Prsoses</span>
